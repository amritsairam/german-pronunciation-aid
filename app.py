...
# # --- OpenAI STT Function with Detailed Logging ---
# def transcribe_audio_openai(audio_path):
#     st.info("Preparing to transcribe with OpenAI...") # UI Log
#     print("\n--- Starting OpenAI Transcription ---") # Terminal Log
#     print(f"Audio file path: {audio_path}") # Terminal Log

#     api_key = os.getenv("OPENAI_API_KEY")

#     if not api_key:
#         st.error("OpenAI API key not found! Please set the OPENAI_API_KEY environment variable.")
#         print("ERROR: OPENAI_API_KEY environment variable not set.") # Terminal Log
#         return None

#     print("OpenAI API key found.") # Terminal Log
    
#     # Check if file exists and has size
#     if not os.path.exists(audio_path) or os.path.getsize(audio_path) == 0:
#         st.error(f"Audio file not found or is empty: {audio_path}")
#         print(f"ERROR: Audio file not found or is empty: {audio_path}")
#         return None
        
#     print(f"Audio file size: {os.path.getsize(audio_path)} bytes.") # Terminal Log

#     try:
#         client = openai.OpenAI(api_key=api_key)

#         with open(audio_path, "rb") as audio_file:
#             st.info("Uploading audio to OpenAI...") # UI Log
#             print("Sending request to OpenAI STT API...") # Terminal Log
#             response = client.audio.transcriptions.create(
#                 model="whisper-1",
#                 file=audio_file,
#                 language="de" # Set language to German
#             )
#             st.info("Received response from OpenAI.") # UI Log
#             print("Received response from OpenAI STT API.") # Terminal Log
#             transcript = response.text
#             print(f"Transcription successful: {transcript}") # Terminal Log
#             print("--- Finished OpenAI Transcription ---") # Terminal Log
#             return transcript

#     except openai.APIConnectionError as e:
#         st.error(f"OpenAI API Connection Error: Could not connect. Check network. Details: {e}")
#         print(f"ERROR: OpenAI API Connection Error: {e}")
#         return None
#     except openai.RateLimitError as e:
#         st.error(f"OpenAI Rate Limit Exceeded: You might be sending requests too quickly. Details: {e}")
#         print(f"ERROR: OpenAI Rate Limit Error: {e}")
#         return None
#     except openai.APIStatusError as e:
#         st.error(f"OpenAI API Status Error: {e.status_code} - {e.response}. Check API key and status.openai.com.")
...
# # --- Other Functions (gTTS, Similarity, Color - as before) ---
# german_phrases = {
...
# # --- Record and Save Audio with Logging ---
...
#             st.info("Processing...") # This is the message you saw before getting stuck
...
#             print("WARNING: WaitTimeoutError - No speech detected.")
#             return None
#         except sr.UnknownValueError:
#             st.warning("Sorry, I could not understand that (microphone).")
#             print("WARNING: UnknownValueError - Could not understand audio.")
#             return None
...
# # ... (rest of the UI code is mostly the same) ...
# selected_phrase_text = st.selectbox("Select a phrase:", list(german_phrases.keys()))

# if st.button("Listen to Phrase 🎧"):
#     audio_file = text_to_speech(german_phrases[selected_phrase_text])
#     if audio_file:
#         st.audio(audio_file)
...
#     print("\n>>> Record and Transcribe Button Clicked <<<")
#     audio_file_path = record_and_save_audio()
#     transcribed_text = None
#     if audio_file_path:
#         print(f"Audio file path received: {audio_file_path}")
#         transcribed_text = transcribe_audio_openai(audio_file_path)
#         print("Transcription attempt finished.")
#         try: 
#             print(f"Attempting to remove temp file: {audio_file_path}")
#             os.remove(audio_file_path)
#             print("Temp file removed.")
#         except Exception as e: print(f"Error removing temp file {audio_file_path}: {e}")

#     if transcribed_text is not None:
#         print("Transcription received, updating session state.")
#         st.session_state.transcribed_text = transcribed_text
#         st.session_state.similarity_score = get_similarity_score(german_phrases[selected_phrase_text], transcribed_text)
#     else:
#         print("Transcription failed or was empty, clearing session state.")
...
# --- OpenAI STT Function (as defined before) ---
